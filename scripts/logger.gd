...
## Usage: Logger.info("Server", "Player spawned", {"player_id": 1, "position": Vector2(0,0)})
...
func _format_message(level_str: String, category: String, message: String, metadata: Dictionary = {}) -> String:
	var parts = [
		"[" + get_timestamp() + "]",
		"[" + level_str + "]",
		"[" + category + "]",
...
func log_snapshot_sent(peer_id: int, sequence: int, entity_count: int, byte_size: int, compression_ratio: float):
...
func log_snapshot_received(sequence: int, entity_count: int, player_id: int, delay_ms: float):
...
func log_entity_disappeared(entity_id: int, last_seen_seq: int, current_seq: int):
...
func log_player_input(peer_id: int, input_dir: Vector2, position: Vector2):
...
func log_chunk_change(entity_id: int, old_chunk: Vector2i, new_chunk: Vector2i, position: Vector2):
...
func log_baseline_mismatch(expected_seq: int, actual_seq: int, snapshot_seq: int):
...
func log_interpolation_warning(warning_type: String, render_time: float, latest_time: float, buffer_size: int):
...
func log_player_disappearance(entity_id: int, snapshot_seq: int, in_snapshot: bool):
...
func log_collision(entity_id: int, collided_with: String, position: Vector2):
...
func log_network_simulation(event_type: String, packet_seq: int, simulated_loss: bool, simulated_lag_ms: int):
